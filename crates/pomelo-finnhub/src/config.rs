...
pub struct SyncConfig {
    ...
    /// Also fetch fundamentals → `fundamentals/{SYM}.csv.gz` (phase #228).
    pub include_fundamentals: bool,
    /// Also fetch sector map → `tracked/universe.csv.gz` (phase #227).
    pub include_industry: bool,
    /// Also compute best-effort snapshot panels (phase #230).
