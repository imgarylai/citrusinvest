...
//!
//! Skeleton (#213): validate inputs only. Price writes land in #214.
...
///
/// **Skeleton (#213):** validates API key, date window, and symbols. Does not
...
pub fn sync_into<H: HttpClient, S: ObjectSink + ObjectSource>(
    ...
    // Wire Fetcher now so throttle/retry paths stay compiled; #214 will fetch.
    ...
    for raw in symbols {
        let Some((layout, _av)) = split_symbol(raw, &cfg.default_exchange) else {
            summary
                .failures
        ...
        any_valid = true;
        // Placeholder until #214 (prices).
        summary.failures.push((
            layout,
            "price sync not implemented yet (pomelo-alpha-vantage skeleton; see #214)".into(),
        ));
    }
    ...
    eprintln!(
        "pomelo-alpha-vantage skeleton: validated {} symbol(s); price/fundies/industry \
         phases land under epic #209",
        symbols.len()
    );
    Ok(summary)
}
...
mod tests {
    ...
    fn skeleton_records_not_implemented_per_symbol() {
        let dir = std::env::temp_dir().join("pomelo_av_skel");
        let summary = sync(
            &NoHttp,
            "demo",
            &["AAPL".into(), "MSFT".into()],
