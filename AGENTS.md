...
- `pomelo-alpha-vantage` — bring-your-own-key Alpha Vantage data sync (epic #209). Same data-layout contract; CLI: `yuzu-cli av-sync` (skeleton first; phases #213–#218).
...
- Published library crates: `yuzu-core`, `yuzu-research`, `pomelo-data`, `pomelo-s3`,
  `pomelo-fmp`, `pomelo-eodhd`, `pomelo-alpha-vantage`, `pomelo-audit`, `lemon-lang` (the
  `lemon` crate is imported as `lemon`). The wasm/CLI/server crates are `publish = false`.
